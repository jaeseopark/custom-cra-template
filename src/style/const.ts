export const APPLE_BIGSUR_GREEN = "#61c454";
export const APPLE_BIGSUR_GREEN_OUTLINE = "#4fa53b";

export const APPLE_BIGSUR_BLUE = "#2f7cf6";
export const APPLE_BIGSUR_BLUE_OUTLINE = "#216fea";

export const APPLE_BIGSUR_RED = "#ed6a5e";
export const APPLE_BIGSUR_RED_OUTLINE = "#d14d40";

export const APPLE_BIGSUR_GRAY_BACKGROUND = "#fafafa";
export const APPLE_BIGSUR_GRAY_OUTLINE = "#c5c5c5";

export const BOX_SHADOW_COLOR = "rgba(0, 0, 0, 0.2)";

export const LARGE_BOX_SHADOW = `0px 10px 35px ${BOX_SHADOW_COLOR}`;
export const MEDIUM_BOX_SHADOW = `0px 5px 20px ${BOX_SHADOW_COLOR}`;
